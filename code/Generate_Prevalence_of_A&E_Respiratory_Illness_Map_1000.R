...
# Query cohort data (A&E Respiratory cases)
cohort_query <- "SELECT LSOA, person_id FROM [CB_PAA_2514].[dbo].[tbl_A&E_Respiratory_distinct]"
...
map_rate <- tm_shape(aggregated_data_sf) +
  tm_polygons("rate_per_1000",
              palette = color_palette,
              title = "A&E Respiratory per 1000 People",
              breaks = rate_breaks,
              colorNA = "grey90") +
  tm_text("LSOA21NM", size = 0.7, col = "black") +
  tm_scale_bar(position = c("left", "bottom")) +
  tm_layout(title = "Prevalence of A&E Respiratory Illness")

# Save each map individually as HTML
tmap_save(tm = map_population, filename = "Total_Population_Map_1000.html")
tmap_save(tm = map_rate, filename = "Prevalence_of_A&E_Respiratory_Illness_Map_1000.html")
